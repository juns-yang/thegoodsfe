...
const RegisterPage = () => {
  ...
  useEffect(() => {
    ...
    } else if (!AgreeAll & AgreeAge & AgreeTermsOfUse & AgreePrivacy & AgreeNewsLetter) {
      SetAgreeAge(false);
  ...
  return (
    <RegisterWrapper>       <LogoWrapper src={logo} />
...
<FormWrapper>
        <InputTextWrapper>이메일<span style={{ color: '#F0C920', marginLeft: '0.26vw'}}>*</span></InputTextWrapper>
        <div style={{display: 'flex', flexDirection: 'row'}}><InputWrapper $width={416}
  ...
  >
          </InputWrapper><DuplicateCheckButton>중복확인</DuplicateCheckButton></div>
          <WarningText>{ValidEmail ? "" : "이메일을 입력해 주세요." }</WarningText>

        <InputTextWrapper>비밀번호<span style={{ color: '#F0C920', marginLeft: '0.26vw'}}>*</span></InputTextWrapper>
        <InputWrapper $width={553} type="password"
        ...
        {ValidPassword ? null : <WarningText>"영문, 숫자, 특수문자를 조합한 8자 이상, 20자 이하의 비밀번호를 입력해주세요."</WarningText>}
        <InputWrapper $width={553} type="password"
        ...
        ></InputWrapper>
          <WarningText>{ValidPasswordCheck ? "" : "비밀번호를 동일하게 입력해주세요." }</WarningText>
        <InputTextWrapper>닉네임<span style={{ color: '#F0C920', marginLeft: '0.26vw'}}>*</span></InputTextWrapper>
        <div style={{display: 'flex',flexDirection: 'row'}}><InputWrapper $width={416}
        ...
        <InputTextWrapper>전화번호<span style={{ color: '#F0C920', marginLeft: '0.26vw'}}>*</span></InputTextWrapper>
        <div style={{display: 'flex',flexDirection: 'row'}}><InputWrapper $width={416}
        onChange={CellChange}
        key="cell"
        placeholder="- 를 제외한 번호만 입력해주세요."
        className={ValidCell ? "":"invalidinput"}
        ></InputWrapper><DuplicateCheckButton
        onClick={() => {SetDisplayVerificationCell(!DisplayVerificationCell);}}>인증요청</DuplicateCheckButton></div>
                <WarningText>{ValidCell ? "" : "정확한 휴대폰번호를 입력해 주세요." }</WarningText>
                {
  DisplayVerificationCell ? (
    <div style={{display: 'flex', flexDirection: 'column', alignItems: 'flex-start'}}>
      ...
      <WarningText>{ValidVerifcationCell ? "" : "전화번호 인증이 필요합니다."}</WarningText>
    </div>
  ) : null
}
        <InputTextWrapper>성별<span style={{ color: '#F0C920', marginLeft: '0.26vw'}}>*</span></InputTextWrapper>
        <GenderSelectionWrapper>
        <GenderSelectionInput type="radio" id="male" name="gender" onClick={GenderChange}/>
      <GenderSelectionLabel htmlFor="male">
        <GenderSelectionText>남자</GenderSelectionText>
      </GenderSelectionLabel>
      <GenderSelectionInput type="radio" id="female" name="gender" onClick={GenderChange}/>
      <GenderSelectionLabel htmlFor="female">
        <GenderSelectionText>여자</GenderSelectionText>
      </GenderSelectionLabel>
      <GenderSelectionInput type="radio" id="noselect" name="gender" onClick={GenderChange}/>
      <GenderSelectionLabel htmlFor="noselect">
        <GenderSelectionText>선택안함</GenderSelectionText>
      </GenderSelectionLabel>
        </GenderSelectionWrapper>
        <WarningText>{Gender}</WarningText>
        <InputTextWrapper>생년월일<span style={{ color: '#F0C920', marginLeft: '0.26vw'}}>*</span></InputTextWrapper>
        ...
        <AgreeWrapper>
        <AgreeInput type="checkbox" id="AgreeAll" name="AgreeAll"
        checked={AgreeAll} 
        onChange={() => SetAgreeAll(AgreeAll => !AgreeAll)}
        />
      <AgreeLabel htmlFor="AgreeAll">
        <AgreeDiv>모두 동의합니다.</AgreeDiv>
      </AgreeLabel>        <AgreeDetailWrapper>
      <AgreeInput type="checkbox" id="AgreeAge" name="AgreeAge"
      checked={AgreeAge}
      onChange={() => SetAgreeAge(AgreeAge => !AgreeAge)} />
      <AgreeLabel htmlFor="AgreeAge" $interval="11">
        <AgreeDiv>만 14세 이상입니다.</AgreeDiv>
      </AgreeLabel>
      <AgreeInput type="checkbox" id="AgreeTermsOfUse" name="AgreeTermsOfUse" 
      checked={AgreeTermsOfUse}
      onChange={() => SetAgreeTermsOfUse(AgreeTermsOfUse => !AgreeTermsOfUse)}/>
      <AgreeLabel htmlFor="AgreeTermsOfUse" $interval="11">
        <AgreeDiv>이용약관 동의</AgreeDiv>
      </AgreeLabel>
      <AgreeInput type="checkbox" id="AgreePrivacy" name="AgreePrivacy"
      ...
      <AgreeInput type="checkbox" id="AgreeNewsLetter" name="AgreeNewsLetter" 
      checked={AgreeNewsLetter}
      onChange={() => SetAgreeNewsLetter(AgreeNewsLetter => !AgreeNewsLetter)}/>
      <AgreeLabel htmlFor="AgreeNewsLetter" $interval="11">
        <AgreeDiv>할인쿠폰/이벤트/감동적인 뉴스레터 선택 동의 (선택)<br /><span style={{ color: '#9C9C9C' , marginTop: '0.73vw'}}>SMS, 이메일을 통해 쿠폰 및 이벤트 정보를 받아보실 수 있습니다.</span></AgreeDiv>
      </AgreeLabel>
        </AgreeDetailWrapper>
        </AgreeWrapper>
        <RegisterButton
